...
#nc_file='Data_noaa_copernicus/noaa_avhrr/noaa_combined_1982_2024.nc'
#nc_file='noaa_combined_1982_2024_1res.nc'
nc_file='noaa_icesmi_combinefile_FINAL_1res1982_2024.nc'
#nc_file = 'noaa_newfile/noaasst1982_2023_data.nc'
#outpath = 'mhw_results/alter_hobday1991_2020'  # Output directory
outpath = 'mhw_results'

# Define climatological baseline period
clim_b = [1991, 2024]  # Updated climatology period
#clim_b = [1982, 2020]
...
mhw_meanintensity = np.nan * np.zeros((len(time_yr), len(j_which), len(i_which)))
...
# Prepare list to store all MHW events from all grid points
all_mhw_events_list = []
...
for i in i_which:
    ...
    for j in j_which:
        ...
        if np.logical_not(np.isfinite(sst1.sum())) or (sst1 < -1).sum() > 0:
            ...
            # Store all events for this grid point
            for ev in range(mhws['n_events']):
                all_mhw_events_list.append({
                'Latitude': lat[j],
                'Longitude': lon[i],
                'Start Date': mhws['date_start'][ev].strftime('%Y-%m-%d'),
                'End Date': mhws['date_end'][ev].strftime('%Y-%m-%d'),
                'Duration (days)': mhws['duration'][ev],
                'Max Intensity (°C)': mhws['intensity_max'][ev],
                'Mean Intensity (°C)': mhws['intensity_mean'][ev],
                 })
            ...
            mhw_meanintensity[:, j, i] = mhwBlock['intensity_mean']
...
# Save full list of MHW events across all grid points
df_all_events = pd.DataFrame(all_mhw_events_list)
df_all_events.to_csv("mhw_results/all_mhw_events_by_grid.csv", index=False)
print("Saved all MHW events to CSV: mhw_results/all_mhw_events_by_grid.csv")
...
ds_out['mhw_meanintensity'] = (('time', dim[-2], dim[-1]), mhw_meanintensity)
...
ds_out['mhw_meanintensity'] = ds_out.mhw_meanintensity.assign_attrs(units=ds.sst.attrs['units'], standard_name="n/a", long_name="Annual mean of marine heatwave intensities in each year (as an anomaly w.r.t. seasonal climatology)", coverage_content_type="auxiliaryInformation")
...
outfile = outpath + '/' + 'mhw_stats_alterhobday34'
#outfile = outpath + '/' + 'mhw_stats_alterhobday39'
