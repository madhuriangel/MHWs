...
# =============================================================================
df_ev_max39_2021 = df_ev_max39[df_ev_max39['Start Date'].dt.year == 2021].copy()
df_ev_dur39_2021 = df_ev_dur39[df_ev_dur39['Start Date'].dt.year == 2021].copy()
...
df_ev_max39_2021['Longitude'] = df_ev_max39_2021['Longitude'].apply(convert_longitude)
df_ev_dur39_2021['Longitude'] = df_ev_dur39_2021['Longitude'].apply(convert_longitude)

# =============================================================================
# 3. LOAD THE EARLIER CSV DATA (HABS / Toxicity Locations)
# =============================================================================
csv_df = pd.read_csv("mhw_analysis_alterhob/hab2021.csv")
...
def filter_by_proximity(df_events, csv_df, threshold_km=100):
    ...
    for idx, event in df_events.iterrows():
        for _, hab in csv_df.iterrows():
            if haversine(event['Longitude'], event['Latitude'],
                         hab['Longitude'], hab['Latitude']) <= threshold_km:
                keep_idx.append(idx)
                break
    # .loc will preserve all the original columns (even if keep_idx is empty)
    return df_events.loc[keep_idx]

df_ev_max39_close = filter_by_proximity(df_ev_max39_2021, csv_df, threshold_km=100)
df_ev_dur39_close = filter_by_proximity(df_ev_dur39_2021, csv_df, threshold_km=100)

# Save filtered DataFrames to CSV (optional)
df_ev_max39_close.to_csv("mhw_analysis_alterhob/plots/strongest_events_2021_close100km.csv", index=False)
df_ev_dur39_close.to_csv("mhw_analysis_alterhob/plots/longest_events_2021_close100km.csv", index=False)
...
ax.set_title("2021 MHW Events Close to Toxicity Locations", fontsize=title_fontsize)
...
plt.savefig('mhw_analysis_alterhob/plots/stronglong_tox2021.png')
plt.show()
