...
outpath = 'mhw_results/hobday1991_2024'
#outpath = 'mhw_results/hobday1991_2020'  # Output directory

# Define climatological baseline period
clim_b = [1991, 2024]
#clim_b = [1982, 2020]  # Updated climatology period
...
for i in i_which:
    ...
    for j in j_which:
        ...
        if np.logical_not(np.isfinite(sst1.sum())) or (sst1 < -1).sum() > 0:
            ...
            mhw_meanintensity[:, j, i] = mhwBlock['intensity_mean']#mean intensity value
...
#outfile = outpath + '/' + 'mhw_stats_hobday30'
outfile = outpath + '/' + 'mhw_stats_hobday34'
